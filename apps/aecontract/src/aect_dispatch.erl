...
call_common(#{gas := Gas, call := Call, trees := Trees, events := Events} = CallDef, Spec) ->
        ...
        InitState ->
            ...
            try aevm_eeevm:eval(InitState) of
                {ok, #{gas := GasLeft, out := Out, chain_state := ChainState}} ->
                    {
                        ...
                        aec_vm_chain:get_events(ChainState)
                    };
                ...
                {error, Error, _} ->
                    ...
                    {create_call(Gas, error, Error, Call), Trees, Events}
            catch T:E ->
                lager:debug("Return error ~p:~p~n", [T,E]),
                {create_call(Gas, error, Call), Trees, Events}
            end
    catch T:E ->
