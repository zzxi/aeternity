...
when_connected(timeout) ->
    ...
    L =
        [{{conn, timeout},
          {stop,
           #{mode => server, phase => disconnected, timer_phase => undefined}}
         }],
    prep_connected(T) ++ [{T, test, E, R} || {E, R} <- L];
when_connected(authorize) ->
    T = <<"when connected - authorize">>,
    L =
        [{{conn, #{type => req, method => authorize, id => 1,
...
when_connected(submit) ->
    ...
    L =
        [{{conn, #{type => req, method => submit, id => 1,
...
when_configured(configure) ->
    ...
    L =
        [{{conn, #{type => req, method => configure, id => 1, params => []}},
          {send,
           #{type => rsp, method => configure, id => 1, reason => unknown_error},
           #{mode => server, phase => configured, timer_phase => configured}}
         }],
    prep_configured(T) ++ [{T, test, E, R} || {E, R} <- L];
...
when_configured(not_req) ->
    ...
    L =
        [{{conn, #{type => rsp, method => configure, id => 2, result => []}},
          {send,
...
when_subscribed(not_req) ->
    ...
    L =
        [{{conn, #{type => rsp, method => configure, id => 3, result => []}},
          {send,
...
when_authorized(not_req) ->
    ...
    L =
        [{{conn, #{type => rsp, method => subscribe, id => 3,
...
conn_init() ->
    {{conn, init},
     {no_send, #{mode => server, phase => connected, timer_phase => connected}}
    }.
...
conn_make_parse_error(Phase, TimerPhase) ->
    {{conn, <<"some random binary">>},
     {send,
      ...
      #{mode => server, phase => Phase, timer_phase => TimerPhase}}
    }.
